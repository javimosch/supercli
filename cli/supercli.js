      ...
      if (!humanMode) {
        output({
          ...
          first_steps: [
            ...
            { command: 'supercli discover --intent "<task>" --json', purpose: 'Find plugins and commands for a task' },
            { command: "supercli plugins explore --name <tool> --json", purpose: "Search plugin registry by keyword" },
            { command: "supercli plugins learn <name> --json", purpose: "Get detailed plugin metadata and commands" },
            { command: "supercli commands --query <keyword> --limit 50 --json", purpose: "Query available commands across all plugins" },
            { command: "supercli inspect <namespace> <resource> <action> --json", purpose: "View command schema — args, types, positional markers" },
            { command: "supercli plan <namespace> <resource> <action> --<args>", purpose: "Preview execution steps and risk before running" },
            { command: "supercli execute <plan_id>", purpose: "Execute a previously persisted plan" },
