...
function writeFakeWacliBinary(dir) {
  ...
  fs.writeFileSync(bin, [
    ...
    "const storeArg = args.find(a => a.startsWith('--store=')) || args.find((a, i) => a === '--store' && i + 1 < args.length ? args[i + 1] : null);",
    "const store = storeArg ? (storeArg.includes('=') ? storeArg.split('=')[1] : args[args.indexOf(storeArg) + 1]) : null;",
    "const storeIndex = args.indexOf('--store');",
    "const clean = args.filter((arg, i) => arg !== '--json' && !arg.startsWith('--store=') && arg !== '--store' && i !== storeIndex + 1);",
